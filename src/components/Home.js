...
function Home() {
  ...
  const searchBreweries = async () => {
    try {
      ...
      if (searchType === 'city') {
        endpoint = `https://api.openbrewerydb.org/v1/breweries?by_city=san_diego&per_page=5`;
      } else if (searchType === 'name') {
        endpoint = `https://api.openbrewerydb.org/v1/breweries?by_name=cooper&per_page=5`;
      } else if (searchType === 'type') {
        endpoint = ` https://api.openbrewerydb.org/v1/breweries?by_type=micro&per_page=5`;
      }
